...
enum class BossDashPurpose
{
    ...
    // Dramatic movement to the chamber centre before a special.
    CenterRoar,
    CenterLaser,
    CenterHealing
};
...
struct Enemy
{
    ...
    // Exact destination used by centre-stage special-attack dashes.
...
private:
    ...
    // The second authored chamber is the dedicated Boss arena.
...
// --------------------------------------------------
    ...
    void LoadMediumEnemySpriteSheet();
    ...
    void DrawMediumEnemySprite(
        const Enemy& enemy,
        Vector2 drawPosition,
        Color tint
    );

    bool GetMediumEnemyAnimationFrame(
        const Enemy& enemy,
...
// returns to frame 0.
    ...
    // Medium enemy (Tank) directional animations.
    Texture2D mediumEnemyIdleSpriteSheet{};
    Texture2D mediumEnemyWalkSpriteSheet{};

    bool mediumEnemyIdleSpriteLoaded = false;
    bool mediumEnemyWalkSpriteLoaded = false;

    int mediumEnemyFrameWidth = 256;
    int mediumEnemyFrameHeight = 256;
    int mediumEnemyDirectionRows = 8;

    int mediumEnemyIdleFramesPerRow = 9;
    int mediumEnemyWalkFramesPerRow = 9;

    float mediumEnemyIdleFrameDuration = 0.14f;
    float mediumEnemyWalkFrameDuration = 0.09f;

    float mediumEnemyVisualScale = 0.58f;
    float mediumEnemyAnchorY = 0.98f;
    ...
    // Normal-enemy health bars are optional.
// The boss HUD is unaffected by this setting.
    bool showNormalEnemyHealthBars = false;
...
// can touch the ground without putting the billboard below it.
    ...
    // Kill-driven progression. The player starts without active skills.
...
// --------------------------------------------------
    ...
    // 256 × 0.90 = approximately 230 pixels tall.boss size
    float bossVisualScale = 1.8f;
...
// --------------------------------------------------
    ...
    // Short punishment after colliding with a wall or obstacle.
    float bossChargeWallStunDuration = 1.20f;
...
// --------------------------------------------------
    ...
    // Fast theatrical repositioning before roar and laser.
    float bossCenterDashSpeed = 1350.0f;
