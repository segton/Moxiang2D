...
# Match the complete three-skill initialization block semantically instead
# of requiring the original cooldowns, unlocked values, comments, or blank
# lines. The local project may already contain earlier combat-speed changes.
skill_init_pattern = re.compile(
    r'^[ \t]*skills\[0\]\.type\s*=\s*SkillType::SpinningBlade;[^\n]*\n'
    r'.*?'
    r'^[ \t]*skills\[2\]\.level\s*=\s*1;[ \t]*$',
    re.MULTILINE | re.DOTALL
)

cpp, skill_replacements = skill_init_pattern.subn(new_skills, cpp, count=1)
if skill_replacements != 1:
    raise RuntimeError(
        'init skills: could not locate the three-skill initialization block '
        'inside Game::InitCombat()'
    )
